...
function setupWeekNavigation() {
    ...
    if (!btnPrev || !btnNext || !titleDisplay)
        return;
...
async function fetchAndRenderWeeklySchedule(childId) {
...
async function updateMonthDisplay(titleDisplay) {
...
async function fetchInfoEvents(year, month) {
...
async function fetchHolidays(year, month) {
    try {
        ...
        return data.dagar
            .filter((dag) => dag["röd dag"] === "Ja" && dag.helgdag)
            .map((dag) => ({
            datum: dag.datum,
            helgdag: dag.helgdag
        }));
...
async function generateMonthView(year, month) {
...
// TEMPLATE FUNCTIONS
async function applyTemplate(templateId, childId) {
    try {
