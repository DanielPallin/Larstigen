...
import { initGlobalUI } from './global.js';
...
async function getInformationPageData() {
    ...
    try {
        ...
        const myChild = firstRelation.child;
...
async function renderInformation(child, notes) {
    const container = document.querySelector('.container');
    if (!container)
        return;
    ...
    const createNoteHtml = (note, type) => `
...
async function fetchMenuData(departmentId) {
...
export async function renderMenuSection(allMenus, childDeptId) {
