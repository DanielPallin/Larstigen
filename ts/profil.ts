...
interface ChildData {
  ...
  child_allergy: { severity: string; allergy: { name: string } | null }[] | null;
  medical_note: { title: string; note: string }[] | null;
  child_caregiver: { caregiver: Caregiver | null }[] | null;
}
...
async function initProfile(): Promise<void> {
  ...
  try {
    ...
    if (cError || !rawCaregiver) throw new Error("Kunde inte hitta profil i databasen.");
    
    const loggedInCaregiver = rawCaregiver as Caregiver;
    ...
    const relations = (rawRelations || []) as unknown as RelationData[];

    const uniqueCaregivers = new Map<string, Caregiver>();
    ...
    for (let i = 0; i < relations.length; i++) {
      ...
      if (c.medical_note && c.medical_note.length > 0) {
        notesHtml = c.medical_note.map(n => 
          `<div class="note-box"><strong>${n.title || 'Info'}:</strong> ${n.note || ''}</div>`
        ).join('');
      }
